DELIMITER /MANGINA/
create or replace procedure `w3hacknet`.`AddContributors` (
IN _repo_id int(11),
IN _json longtext
)
BEGIN
	...
	while _idx < _len DO
		...
		if _alias_id > 0 then
			call debug(concat('updating ', _alias_id, ' incrementing count by ', _count));
			update alias set count = count + _count where id = _alias_id;
		else
			call debug(concat('inserting ', _user_id, ' initial count of ', _count));
