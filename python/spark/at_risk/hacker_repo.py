...
out_sql = """
SELECT rh.owner, rh.repo_name, rh.partition_key, rh.owner_repo,
       regexp_extract(rh.author, '(.*) <(.*)@(.*)>', 1) AS author_name,
       regexp_extract(rh.author, '(.*) <(.*)@(.*)>', 3) AS author_email_domain,
       md5(regexp_extract(rh.author, '(.*) <(.*)>', 2)) AS author_email_hash
FROM curated_repo_hacker rh
WHERE EXISTS (
  SELECT 1 FROM athena_targetable_hackers ah
    WHERE ah.author = rh.author
)
"""

log.info(f'executing sql:\n{out_sql}')
out_df = spark.sql(out_sql).coalesce(1)
insert_or_update(out_df, 'deadmandao-at-risk', 'web_data', 'repo_hacker')
